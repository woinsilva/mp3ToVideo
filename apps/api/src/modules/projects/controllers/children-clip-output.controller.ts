...
import { GenerateChildrenClipVideoDto } from '../dtos/generate-children-clip-video.dto';
...
export class ChildrenClipOutputController {
  ...
  @Post('shots/:shotId/generate') hero(@CurrentUser() user: AuthenticatedUser, @Param('projectId') projectId: string, @Param('shotId') shotId: string, @Body() input: GenerateChildrenClipVideoDto) { return this.output.generateHero(projectId, shotId, user.organizationId, user.userId, input); }
  @Post('shots/:shotId/wan') legacyWan(@CurrentUser() user: AuthenticatedUser, @Param('projectId') projectId: string, @Param('shotId') shotId: string) { return this.output.generateHero(projectId, shotId, user.organizationId, user.userId, { provider: 'local' }); }
  @Post('hero-attempts/:attemptId/retry') retryHero(@CurrentUser() user: AuthenticatedUser, @Param('projectId') projectId: string, @Param('attemptId') attemptId: string, @Body() input: GenerateChildrenClipVideoDto) { return this.output.retryHero(projectId, attemptId, user.organizationId, user.userId, input); }
  ...
  @Get('references/:assetId/file') async referenceFile(@CurrentUser() user: AuthenticatedUser, @Param('projectId') projectId: string, @Param('assetId') assetId: string, @Res() response: Response) { const file = await this.output.getReferenceFile(projectId, assetId, user.organizationId); response.setHeader('Content-Type', file.mimeType); return response.sendFile(file.absolutePath); }
